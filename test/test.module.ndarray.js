...
tape( 'a module instance has an `ndarray` method which calculates the sum of strided array elements (ignoring NaN values)', function test( t ) {
	...
	v = mod.ndarray( 11, xp, 1, 0 );
	t.strictEqual( v, 3.0, 'returns expected value' );

	mod.write( xp, new Float64Array( [ 1.0, -2.0, -4.0, NaN, 5.0, 0.0, 3.0 ] ) );
	v = mod.ndarray( 7, xp, 1, 0 );
	t.strictEqual( v, 3.0, 'returns expected value' );

	mod.write( xp, new Float64Array( [ -4.0, -4.0 ] ) );
	v = mod.ndarray( 2, xp, 1, 0 );
	...
	v = mod.ndarray( 4, xp, 1, 0 );
	t.strictEqual( v, 2.0, 'returns expected value' );

	t.end();
});

tape( 'a module instance has an `ndarray` method which preserves the sign of zero', function test( t ) {
	var mem;
	var mod;
	var xp;
	var v;
	...
	mod.write( xp, new Float64Array( [ -0.0, -0.0, -0.0 ] ) );

	v = mod.ndarray( 3, xp, 1, 0 );
